var vm = new Vue({
	...
	methods: {
		...
		update: function (id) {
			...
			$.get("../sys/${clazzinfo.tableas}/info/"+id, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.${clazzinfo.tableas} = r.${clazzinfo.tableas};
            });
		},
		del: function (id) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: "../sys/${clazzinfo.tableas}/delete",
				    data: {
				    	"id":id
				    },
		...
		saveOrUpdate: function (event) {
			var url = vm.${clazzinfo.tableas}.id == null ? "../sys/${clazzinfo.tableas}/save" : "../sys/${clazzinfo.tableas}/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.${clazzinfo.tableas}),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(index){
							vm.reload();
...
function paging(syc) {
	...
	$.ajax({
		type : "POST",
		url : "../sys/${clazzinfo.tableas}/list",
		contentType : "application/x-www-form-urlencoded",
		dataType : 'json',
		async : syc,
		data : {
