var vm = new Vue({
	...
	methods: {
		...
		update: function (id) {
			...
			$.get("../sys/leaveMessage/info/"+id, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.leaveMessage = r.leaveMessage;
            });
		},
		del: function (id) {
		...
		saveOrUpdate: function (event) {
			var url = vm.leaveMessage.id == null ? "../sys/leaveMessage/save" : "../sys/leaveMessage/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.leaveMessage),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(index){
							vm.reload();
