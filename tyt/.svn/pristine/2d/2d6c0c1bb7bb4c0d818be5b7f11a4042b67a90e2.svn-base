var vm = new Vue({
	...
	methods: {
		...
		update: function (id) {
			...
			$.get("../sys/webDomain/info/"+id, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.webDomain = r.webDomain;
            });
		},
		del: function (id) {
		...
		saveOrUpdate: function (event) {
			var url = vm.webDomain.id == null ? "../sys/webDomain/save" : "../sys/webDomain/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.webDomain),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(index){
							vm.reload();
