var vm = new Vue({
	...
	methods: {
		...
		update: function (id) {
			...
			$.get("../sys/userLine/info/"+id, function(r){
                vm.showList = false;
                vm.title = "修改";
                vm.userLine = r.userLine;
            });
		},
		del: function (id) {
		...
		saveOrUpdate: function (event) {
			var url = vm.userLine.id == null ? "../sys/userLine/save" : "../sys/userLine/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.userLine),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(index){
							vm.reload();
