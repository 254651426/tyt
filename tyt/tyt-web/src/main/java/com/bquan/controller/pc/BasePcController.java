...
public class BasePcController {
	 ...
	 * @return
	 */
	public String outputpcAes(HttpServletResponse response,String obj) {
		response.setHeader("Access-Control-Allow-Origin","*");
		response.setHeader("Access-Control-Allow-Methods", "POST, GET, OPTIONS, DELETE");  
		response.setHeader("Access-Control-Max-Age", "3600");  
		response.setHeader("Access-Control-Allow-Headers", "x-requested-with");
...
//			updateVipTime(user, sendMinutes);
	...
	public void logout(User user){
		try {
	        ...
	        if(StringUtils.isEmpty(user.getLevel())){
	        	if(userTokenList!=null&&userTokenList.size()>0){
	            	for(UserToken uk:userTokenList){
	            		...
	            		redisUtil.del(uk.getToken()+"list");
	            		redisUtil.del(uk.getToken()+"user");
	            	}
	            }
	        }else{
	        	// 年费会员支持同时在线两台设备
	        	if("4".equals(user.getLevel())){
	            	...
	            	if(userTokenList.size()>=2){
	            		for(int i=0;i<(userTokenList.size()-1);i++){
	                		...
	                		redisUtil.del(userTokenList.get(i).getToken()+"list");
		            		redisUtil.del(userTokenList.get(i).getToken()+"user");
	                	}
	            	}
	        ...
	        if(userTokenList!=null){
	        	for(UserToken uto:userTokenList){
	        		List<UserToken> utList = new ArrayList<UserToken>();
	        		utList.add(uto);
	        		redisUtil.setoj(uto.getToken()+"list", utList);
	        	}
	        }
	...
	/**
	 * 赠送vip时间 一个设备号只能赠送一次时间 2小时
	 * 
	 * @param user
	 */
	public void sendTime(String phonenum) {
		List<User> user = userReadService.selectphonenum(phonenum);
		boolean isok = false;
		for (User u : user) {
			// 首次赠送
			if (u.getSendDate() == null) {
				isok = true;
			} else {
				isok = false;
				break;
		...
		if (isok) {
			CommissionRate commissionRate = commissionRateReadService.getByUser("first");
			updateVipTime(user.get(0), commissionRate.getLimitSendTime());
		}
