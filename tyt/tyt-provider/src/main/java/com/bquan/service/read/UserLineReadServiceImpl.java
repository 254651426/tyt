...
public class UserLineReadServiceImpl extends BaseReadServiceImpl<UserLine> implements UserLineReadService{
	...
	public UserLineP getAllLine(UserLine pidline) {
		  UserLineP up = new UserLineP();
		  up.setArea(pidline.getArea());
		  up.setId(pidline.getId());
		  List<UserLine> cid = 	userLineReadMapper.queryListParentId(pidline.getId());
		  up.setUserLinelist(cid);
		  return up;
	}
